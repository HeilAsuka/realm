...
pub async fn transfer_udp(
    ...
    loop {
        ...
        let remote_addr = format!("{}:{}", remote_ip.read().await, remote_port)
            .parse::<SocketAddr>()
            .unwrap();

        // the socket associated with a unique client
        let alloc_sock = match get_socket(&sock_map, &client_addr).await {
            Some(x) => x,
            None => {
                alloc_new_socket(&sock_map, client_addr, &remote_addr, local_sock.clone()).await
            }
        };
...
async fn send_back(
    ...
    while let Ok(Ok((n, _))) = timeout(TIMEOUT, alloc_sock.recv_from(&mut buf)).await {
        if local_sock.send_to(&buf[..n], &client_addr).await.is_err() {
            break;
        }
    }

    sock_map.write().await.remove(&client_addr);
}

#[inline]
async fn get_socket(sock_map: &SockMap, client_addr: &SocketAddr) -> Option<Arc<UdpSocket>> {
    let alloc_sock = sock_map.read().await;
